...
configuration as required, e.g. `geopython`. This configuration can be activated at any
point, for example:

```sh
gcloud config set project geopython
...
```sh
python -m ipykernel install --user --name geopython --display-name "Python (geopython)"
```
